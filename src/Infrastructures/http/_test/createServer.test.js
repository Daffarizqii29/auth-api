...
const registerAndLogin = async (app, {
  ...
  const authenticationResponse = await request(app).post('/authentications').send({ username, password });
  return authenticationResponse.body.data.accessToken;
};
...
describe('HTTP server', () => {
  ...
  describe('when GET /threads/:threadId', () => {
    ...
    it('should mask deleted comments and replies', async () => {
      const app = await createServer(container);
      const accessToken = await registerAndLogin(app);
      const threadResponse = await request(app)
        .post('/threads')
